...
BRAND       = "@ʜᴇɴʀʏ #тωιηѕ(oғғ) нoѕтιηg"
BRAND_VER   = "v2.1"
BRAND_TAG   = f"{BRAND} {BRAND_VER}"
SUPPORT_USR = "@nur7871"
UPDATE_CH   = "https://t.me/+MXtA9ufCgok3Yjc1"
class _DynamicFooter:
    """Renders using the CURRENT BRAND_TAG every time it's used, instead of
    freezing the value at import time. Supports the string-like operations
    (concatenation, len(), slicing) used on FOOTER elsewhere in this file."""
    def _val(self) -> str:
...
class _EmojiDict(dict):
    """Glyph lookup order: 1) admin's saved custom_emojis override
    (Appearance → Custom Emojis), 2) built-in premium emoji set below,
    3) plain unicode default. Making G a plain dict meant custom emoji
    edits were saved but never actually shown anywhere — this fixes that."""
    def __getitem__(self, key):
        custom = get_setting("custom_emojis", {}) or {}
        if key in custom and custom[key]:
            return custom[key]
        if key in _PREMIUM_EMOJIS:
            return _PREMIUM_EMOJIS[key]
...
# Premium (Telegram Premium) custom emoji defaults. These render as
# animated custom emoji for viewers with Premium, and fall back to the
# plain emoji inside the tag for everyone else. Owner's account must have
# an active Premium subscription for the bot to be able to send these.
# NOTE: pure line-divider glyphs (div/div_eq/div_dash/border_*/block_*)
# are intentionally left as plain unicode — they're repeated 16x to draw
# separator lines, and repeating a custom-emoji tag that many times in a
# row renders as a broken wall of icons instead of a clean line.
_PREMIUM_EMOJIS: Dict[str, str] = {
    # core status / decisions
    "ok":         '<tg-emoji emoji-id="5219899949281453881">✅</tg-emoji>',
    "no":         '<tg-emoji emoji-id="5219805369806629055">🚫</tg-emoji>',
    "warn":       '<tg-emoji emoji-id="5220197908342648622">❗</tg-emoji>',
    "arrow":      '<tg-emoji emoji-id="5301096984617166561">☝️</tg-emoji>',
    "bullet":     '<tg-emoji emoji-id="5222108309795908493">✨</tg-emoji>',
    "tri":        '<tg-emoji emoji-id="5246794802560774143">🔝</tg-emoji>',
    "diamond":    '<tg-emoji emoji-id="5264892613630111886">💎</tg-emoji>',
    "star":       '<tg-emoji emoji-id="5310224206732996002">⭐</tg-emoji>',
    "spark":      '<tg-emoji emoji-id="5294017134756636818">🌟</tg-emoji>',
    "back":       '<tg-emoji emoji-id="5301275719681190738">〰️</tg-emoji>',
    "fwd":        '<tg-emoji emoji-id="5217890643321300022">✈️</tg-emoji>',
    "plus":       '<tg-emoji emoji-id="5294524383279198295">🆕</tg-emoji>',
    "minus":      '<tg-emoji emoji-id="5278619986238122736">🔇</tg-emoji>',
    "rec":        '<tg-emoji emoji-id="5294096239464295059">🔵</tg-emoji>',
    "rec_off":    '<tg-emoji emoji-id="5278619986238122736">🔇</tg-emoji>',

    # process state
    "play":        '<tg-emoji emoji-id="5258217809250372293">🎥</tg-emoji>',
    "stop":        '<tg-emoji emoji-id="5339233635620899144">⛔</tg-emoji>',
    "pause":       '<tg-emoji emoji-id="5334811859415477124">💤</tg-emoji>',
    "refresh":     '<tg-emoji emoji-id="5292226786229236118">🔄</tg-emoji>',
    "running":     '<tg-emoji emoji-id="5339135753316222622">🟢</tg-emoji>',
    "stopped":     '<tg-emoji emoji-id="5339233635620899144">⛔</tg-emoji>',
    "restarting":  '<tg-emoji emoji-id="5452002073606384268">🔄</tg-emoji>',
    "stop_bot":    '<tg-emoji emoji-id="5339233635620899144">⛔</tg-emoji>',

    # security / access
    "lock":     '<tg-emoji emoji-id="5393302369024882368">🔒</tg-emoji>',
    "unlock":   '<tg-emoji emoji-id="5341492148468465410">📂</tg-emoji>',
    "secure":   '<tg-emoji emoji-id="5391034312759980875">🥷</tg-emoji>',
    "key":      '<tg-emoji emoji-id="5454386656628991407">🔑</tg-emoji>',
    "shield":   '<tg-emoji emoji-id="5393302369024882368">🔒</tg-emoji>',
    "ban":      '<tg-emoji emoji-id="5453991094435997597">⚔️</tg-emoji>',
    "trash":    '<tg-emoji emoji-id="5357592447557848986">💀</tg-emoji>',
    "eye":      '<tg-emoji emoji-id="5220070652756635426">👀</tg-emoji>',

    # people
    "user":   '<tg-emoji emoji-id="5454371323595744068">🥸</tg-emoji>',
    "users":  '<tg-emoji emoji-id="5454371323595744068">🥸</tg-emoji>',
    "crown":  '<tg-emoji emoji-id="5264892613630111886">💎</tg-emoji>',

    # money / commerce
    "wallet":   '<tg-emoji emoji-id="5237761614458933049">🪙</tg-emoji>',
    "premium":  '<tg-emoji emoji-id="5343636681473935403">💎</tg-emoji>',
    "lifetime": '<tg-emoji emoji-id="5341601055954194475">⭐</tg-emoji>',
    "gift":     '<tg-emoji emoji-id="5258212320282168974">🎈</tg-emoji>',
    "ticket":   '<tg-emoji emoji-id="5258212320282168974">🎈</tg-emoji>',
    "trophy":   '<tg-emoji emoji-id="5341684837881235158">⭐</tg-emoji>',

    # data / analytics
    "graph":    '<tg-emoji emoji-id="5454079785510660283">💻</tg-emoji>',
    "stats":    '<tg-emoji emoji-id="5454079785510660283">💻</tg-emoji>',
    "chart_up": '<tg-emoji emoji-id="5246794802560774143">🔝</tg-emoji>',
    "plan":     '<tg-emoji emoji-id="5454419255430767770">📎</tg-emoji>',

    # comms
    "broadcast": '<tg-emoji emoji-id="5453965363286925977">📞</tg-emoji>',
    "chat":      '<tg-emoji emoji-id="5220046725493828505">✍️</tg-emoji>',

    # storage / files
    "folder":   '<tg-emoji emoji-id="5341492148468465410">📂</tg-emoji>',
    "upload":   '<tg-emoji emoji-id="5217890643321300022">✈️</tg-emoji>',
    "download": '<tg-emoji emoji-id="5258024802010026053">🛒</tg-emoji>',
    "cloud":    '<tg-emoji emoji-id="5219672809936006424">❄️</tg-emoji>',

    # tools / time / energy
    "settings": '<tg-emoji emoji-id="5258023599419171861">🔧</tg-emoji>',
    "cog":      '<tg-emoji emoji-id="5258023599419171861">🔧</tg-emoji>',
    "bolt":     '<tg-emoji emoji-id="5219943216781995020">⚡</tg-emoji>',
    "clock":    '<tg-emoji emoji-id="5258113901106580375">⌛</tg-emoji>',
}

G = _EmojiDict(_G_DEFAULTS)
...
PAYMENT_METHODS: Dict[str, Dict[str, Any]] = {
    "upi":     {"name": "UPI",           "number": "manutdxsen@ptaxis",                              "type": "UPI ID",              "tag": "[UPI]"},
    "binance": {"name": "Binance Pay",   "number": "Binance ID 1247951455",                           "type": "Binance Pay",         "tag": "[BP]"},
    "usdt":    {"name": "USDT (BEP20)",  "number": "0x11F33864b935F0487eE5C64619fC5a68fc325979",       "type": "USDT Wallet Address", "tag": "[USDT]"},
}
...
def bullet(label: str, value: Any, glyph: str = None) -> str:
    if glyph is None:
        glyph = G["bullet"]
...
class KeyRing:
    ...
    def _gh_request(self, method: str, path: str, **kw) -> Optional[requests.Response]:
        ...
        h.setdefault("User-Agent", "henrytwins-hosting-rbot/2.1")
        try:
...
def render_admin_subroute(call: types.CallbackQuery, data: str) -> None:
    ...
    if data.startswith("adm_bc_set_currency_") and len(data.split("_")) >= 6:
        ...
        if len(parts) == 2:
            set_setting("payment_currency", parts[0]); set_setting("currency_symbol", parts[1])
...
def _gh_api(endpoint: str, token: Optional[str] = None,
    ...
    req.add_header("User-Agent",    "HenryTwinsHostingBot/2.0")
    if body:
...
def action_adm_wh_test(call: types.CallbackQuery) -> None:
    ...
    def _bg() -> None:
        try:
            import urllib.request as _ur
            import json as _j
            payload = _j.dumps({"test": True, "ts": ts_iso(), "from": "HenryTwinsHostingBot"}).encode()
...
def _do_export_data(admin_uid: int) -> Path:
    ...
    target = out / f"henrytwins_export_{stamp}.zip"
    ...
    try:
        ...
        if flow == "await_adm_bc_set":
            ...
            try:
                ...
                val_store = val
            if key in ("payment_currency", "currency_symbol"):
...
_HELP_PAGES = {
    "main": {
        "title": "📚 Help Centre",
        "text": (
            "Welcome to <b>@ʜᴇɴʀʏ #тωιηѕ(oғғ) нoѕтιηg</b>\n\n"
            "<b>Quick Start</b>\n"
...
def action_adm_export_full_db(call):
    ...
    fname = f"henrytwins_db_{now_utc().strftime('%Y%m%d_%H%M%S')}.json"
    import io
...
def action_adm_export_users_csv(call):
    ...
    fname = f"henrytwins_users_{now_utc().strftime('%Y%m%d')}.csv"
    import io
    bot.send_document(call.message.chat.id, (fname, io.BytesIO(data)),
...
def action_adm_export_bots_csv(call):
    ...
    fname = f"henrytwins_bots_{now_utc().strftime('%Y%m%d')}.csv"
    import io
    bot.send_document(call.message.chat.id, (fname, io.BytesIO(data)),
...
def action_adm_export_trans_csv(call):
    ...
    fname = f"henrytwins_transactions_{now_utc().strftime('%Y%m%d')}.csv"
    import io
    bot.send_document(call.message.chat.id, (fname, io.BytesIO(data)),
...
def action_adm_export_audit_csv(call):
    ...
    fname = f"henrytwins_audit_{now_utc().strftime('%Y%m%d')}.csv"
    import io
    bot.send_document(call.message.chat.id, (fname, io.BytesIO(data)),
    ...
    try:
        ...
        with target.open("rb") as fh:
            bot.send_document(
                ...
                parse_mode="HTML",
                visible_file_name=f"henrytwins_backup_{stamp}.zip",
            )
...
def render_referral(call: types.CallbackQuery) -> None:
    ...
    try:
        ...
        link = f"https://t.me/ishowonel?start={uid}"
    cap = (
        f"<b>{G['users']} {sc('Referral')}</b>\n"
        f"{G['div_eq']}\n"
        f"{bullet('Your link', link)}\n"
